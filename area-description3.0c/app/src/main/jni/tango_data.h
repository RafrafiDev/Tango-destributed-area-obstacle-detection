...
Loc(){};
Loc(float la,float lo): lat(la),lon(lo){}
...
class TangoData {
 public:
  ...
  bool SetConfig(bool is_learning, bool is_load_adf,const char *filename);
  ...
  Loc GetNameMetadataValue(std::string uuid);
  ...
  std::vector<std::string> close_adf_list;
  std::vector<std::string> adf_list;

  std::vector<glm::vec3  > pos_adf;
  std::vector<glm::vec3  > pos_astar;
  bool is_drawed=false;
  std::vector<glm::vec3  > pos_astar_sol;

  glm::vec3 pos_min;
  glm::vec3 pos_max;


  std::vector<Loc> loc_list;
  ...
  bool LoadRangeUUIDFile(const char *name,int maxdist);
