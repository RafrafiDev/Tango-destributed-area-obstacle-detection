...
void TangoData::UpdatePoseData() {
  ...
  //__android_log_print(ANDROID_LOG_INFO, "rotation quaternion ", "%f , %f , %f , %f", tango_rotation[0],tango_rotation[1],tango_rotation[2],tango_rotation[3]);
...
//Coordinate according to device
//Yaw X
//Roll Y
//Pitch Z
    float Yaw=-5.0f, Pitch=-5.0f, Roll=-5.0f;
    getEulerRotation(&Yaw, &Pitch, &Roll,rotation);

//__android_log_print(ANDROID_LOG_INFO, "EulerRotation  ",   "Yaw %f Pitch %f Roll %f \n", Yaw, Pitch, Roll);
...
tcpdata_size=(int)(depth_buffer_size*3);
tcpdata =new float [tcpdata_size+8];
//point no
tcpdata[0]=tcpdata_size;
//rotation
tcpdata[2]=Yaw;
tcpdata[3]=Pitch;
tcpdata[4]=Roll;
//translation
tcpdata[5]=translation.x;
tcpdata[6]=translation.y;
tcpdata[7]=translation.z;

  // Calculating average depth for debug display.
  float total_z = 0.0f;
  for (uint32_t i = 0; i < tcpdata_size; i+=3) {
//__android_log_print(ANDROID_LOG_INFO, "tcpdata  ",   "     %f \n", depth_buffer[i]);
    tcpdata[8+i]=depth_buffer[i];
    tcpdata[9+i]=depth_buffer[i+1];
    tcpdata[10+i]=depth_buffer[i+2];

    // The memory layout is x,y,z,x,y,z. We are accumulating
    // all of the z value.
    total_z += depth_buffer[i + 2];
  }
  if (depth_buffer_size != 0) {
    depth_average_length = total_z / static_cast<float>(depth_buffer_size);
  }
//zaverage
tcpdata[1]=depth_average_length;

 //if(TangoData::GetInstance().tcpdata_size>0 && TangoData::GetInstance().is_xyzij_dirty)

//TODO renable
   if(Yaw>0.4f && Yaw<1.1f){
        std::clock_t current = std::clock();
        if(current - start>500000)
        {
     //       __android_log_print(ANDROID_LOG_INFO, "timems  ",   " %d \n", tcpdata_size+8);
            tcpsend(tcpdata,tcpdata_size+8);
            start=current;
        }
    }else setTcpresult((char*) "er");
  delete[] tcpdata;
