...
inline void getEulerRotation(float* Yaw, float* Pitch, float* Roll,glm::quat q)
    {

 *Yaw = atan2(2.0*(q.y*q.z + q.w*q.x), q.w*q.w - q.x*q.x - q.y*q.y + q.z*q.z);
*Pitch = asin(-2.0*(q.x*q.z - q.w*q.y));
*Roll = atan2(2.0*(q.x*q.y + q.w*q.z), q.w*q.w + q.x*q.x - q.y*q.y - q.z*q.z);
//__android_log_print(ANDROID_LOG_INFO, "rotation", "Yaw %.2f Pitch %.2f Roll %.2f",*Yaw,*Pitch ,*Roll);
