...
bool SetupGraphics(int w, int h) {
  ...
  screen_height = h;
/*
char* t=(char*)"tesssssssssssssssssssssssssssssssssssssssssssssssssssss";
__android_log_print(ANDROID_LOG_INFO, "sometag", "%s", t);
*/
...
bool RenderFrame() {
  ...
  if (camera_type == CameraType::FIRST_PERSON) {
    ...
    // cam_cur_angle[1] is the y-axis touch, cooresponding to x-axis rotation.
...
Java_com_projecttango_experiments_nativepointcloud_TangoJNINative_tcpConnect(
JNIEnv*, jobject)
{
  if(tcpconnection("141.26.71.117",60588))
  LOGI("Tango Tcp soket connection established");
    else LOGE("Tango Tcp soket connection Failed");
}



JNIEXPORT void JNICALL
Java_com_projecttango_experiments_nativepointcloud_TangoJNINative_tcpDisconnect(
JNIEnv*, jobject)
{
tcpdisconnect();
}
...
JNIEXPORT void JNICALL
Java_com_projecttango_experiments_nativepointcloud_TangoJNINative_setFloatDepthData
(JNIEnv* env, jobject jobj, jfloatArray arr, jint lenght)
{

    jfloat* tmp= env->GetFloatArrayElements( arr,0);

    float test[lenght];
    test[0] = tmp[4];
//__android_log_print(ANDROID_LOG_INFO, "testeee", "%f", test[0]);
...
Java_com_projecttango_experiments_nativepointcloud_TangoJNINative_getDoubleDepthData
(JNIEnv* env, jobject jobj, float far)
{
 pthread_mutex_lock(&TangoData::GetInstance().xyzij_mutex);
    if(TangoData::GetInstance().depth_buffer!=nullptr && TangoData::GetInstance().depth_buffer_size>0){
        uint32_t size=TangoData::GetInstance().depth_buffer_size * 3;

    std::vector<double> arr;
     for(int i=0;i<size;i+=3){
        if(TangoData::GetInstance().depth_buffer[i+2]<0.8){
         arr.push_back(TangoData::GetInstance().depth_buffer[i]);
         arr.push_back(TangoData::GetInstance().depth_buffer[i+1]);
         arr.push_back(TangoData::GetInstance().depth_buffer[i+2]);
        }
     }

    jdouble tmp[arr.size()];
    for(int j=0;j<arr.size();j++){;
        tmp[j] = arr[j];
    }
    jdoubleArray res;
    res = (jdoubleArray) (env)->NewDoubleArray(arr.size());
    if (res == NULL)  {pthread_mutex_unlock(&TangoData::GetInstance().xyzij_mutex); return NULL;}

    //__android_log_print(ANDROID_LOG_INFO, "sometag", "arr %d", arr.size());
    //__android_log_print(ANDROID_LOG_INFO, "sometag", "tmp %d", arr.size());
    env->SetDoubleArrayRegion((jdoubleArray)res, 0, arr.size(), tmp);
    //__android_log_print(ANDROID_LOG_INFO, "tango++", "%f", tmp[size-1]);
    pthread_mutex_unlock(&TangoData::GetInstance().xyzij_mutex);
    return res;
    }else {
    pthread_mutex_unlock(&TangoData::GetInstance().xyzij_mutex);
   //__android_log_print(ANDROID_LOG_INFO, "sometag", "%f", 0.0f);
    return NULL;
   }
}
...
Java_com_projecttango_experiments_nativepointcloud_TangoJNINative_getStringDepthData
(JNIEnv* env, jobject jobj)
{
  ...
  std::ostringstream buff;
    ...
    uint32_t size=TangoData::GetInstance().depth_buffer_size;
    float * fbuffer=TangoData::GetInstance().depth_buffer;
     if(fbuffer!=nullptr && size>0){

     //glm::mat4 oc_2_ow_mat_depth_1 = glm::mat4(1.0f);
     //oc_2_ow_mat_depth_1=TangoData::GetInstance().d_2_ss_mat_depth;



  float depth_average = TangoData::GetInstance().depth_average_length;




    TangoPoseData pose= TangoData::GetInstance().cur_pose_data;
      float Yaw, Pitch, Roll;
            getEulerRotation(&Yaw, &Pitch, &Roll,glm::quat(pose.orientation[3],pose.orientation[0],
 ...
 /*
      ...
      /*ss_2_ow_mat * temp * glm::inverse(d_2_imu_mat) * c_2_imu_mat *
                 oc_2_c_mat;*/

        // glm::vec4 tmp= ss_2_ow_mat*oc_2_ow_mat_depth_1*glm::inverse(TangoData::GetInstance().d_2_imu_mat)*TangoData::GetInstance().c_2_imu_mat*oc_2_c_mat

         //*glm::vec4(TangoData::GetInstance().depth_buffer[i],TangoData::GetInstance().depth_buffer[i+1], TangoData::GetInstance().depth_buffer[i+2], 1.0);


          //glm::vec4 tmp= glm::vec4(TangoData::GetInstance().depth_buffer[i],TangoData::GetInstance().depth_buffer[i+1], TangoData::GetInstance().depth_buffer[i+2], 1.0);
         // buff<<tmp[0]<<";"<<tmp[1]<<";"<<tmp[2]<<"\n";

          buff<<fbuffer[i]<<";"<<fbuffer[i+1]<<";"<<fbuffer[i+2]<<"\n";
           }
        }
        //buff<<"1.0;1.0;1.0";
//__android_log_print(ANDROID_LOG_INFO, "tango++", "%s", buff.str().c_str());
pthread_mutex_unlock(&TangoData::GetInstance().pose_mutex);
pthread_mutex_unlock(&TangoData::GetInstance().xyzij_mutex);

  return (env)->NewStringUTF(buff.str().c_str());
}


JNIEXPORT jfloatArray JNICALL
Java_com_projecttango_experiments_nativepointcloud_TangoJNINative_getFloatDepthData
(JNIEnv* env, jobject jobj)
{
  ...
  pthread_mutex_lock(&TangoData::GetInstance().xyzij_mutex);
    ...
    if(TangoData::GetInstance().depth_buffer!=nullptr && TangoData::GetInstance().depth_buffer_size>0){
        uint32_t size=TangoData::GetInstance().depth_buffer_size;
        jfloatArray res;
        res = (jfloatArray) (env)->NewFloatArray(size);
        if (res == NULL)  {pthread_mutex_unlock(&TangoData::GetInstance().xyzij_mutex); return NULL;}


    jfloat tmp[size];
    for(int i=0;i<size;i++)
        tmp[i] = TangoData::GetInstance().depth_buffer[i];

    pthread_mutex_unlock(&TangoData::GetInstance().xyzij_mutex);

    env->SetFloatArrayRegion((jfloatArray)res, 0, size, tmp);
    //__android_log_print(ANDROID_LOG_INFO, "tango++", "%f", tmp[size-1]);
    return res;
    }else {
    pthread_mutex_unlock(&TangoData::GetInstance().xyzij_mutex);
...
Java_com_projecttango_experiments_nativepointcloud_TangoJNINative_getDecision(
    JNIEnv*, jobject) {
  return decide();
}
